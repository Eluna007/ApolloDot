...
Apollo presents clipboard history through the public `key clipboard` JSON protocol.
...
- cliphist stores the selected bytes and owns deduplication, history limits and
  deletion. There is no Apollo clipboard database or MIME sidecar.
...
This is not a multi-MIME archive, and Apollo must not describe it as one.
