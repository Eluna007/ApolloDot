...
are distributed as `apollo-shell`, `key-cli` and `keytop`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Eluna007/ApolloDot/main/install.sh | bash
```
...
Apollo uses the exact release that generated the installer. key-cli and keytop each resolve
...
```bash
curl -fsSL https://raw.githubusercontent.com/Eluna007/ApolloDot/main/install.sh | bash -s -- --dry-run
```
...
- Enable Apollo Shell with the Niri user service.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Eluna007/ApolloDot/main/install.sh | bash -s -- \
  --non-interactive \
...
restarting or asking again. Choices are saved under `$XDG_STATE_HOME/apollo/installer/choices.json`
...
System files are installed under `/etc/xdg/quickshell/apollo`, `/usr/lib/qt6/qml/Apollo`,
and `/usr/lib/systemd/user`. User configuration remains in the existing XDG locations.
`~/.config/quickshell/apollo`, `/usr/local/bin/key`, development venvs, environment overrides
...
verify the GitHub Release build assets for `key-cli`, `keytop`, and `apollo-shell` in that order.
