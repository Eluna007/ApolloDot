...
umask 077
snapshot_dir=$(mktemp -d "${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}/apollo-snapshot.XXXXXX")
trap 'rm -rf -- "$snapshot_dir"' EXIT
trap 'exit 1' HUP INT TERM
export APOLLO_SNAPSHOT_OUTPUT="$output"
export APOLLO_SNAPSHOT_PATH="$snapshot_dir/frame.bmp"
timeout --kill-after=0.2s 1.5s quickshell --path "$script_dir/LockSnapshot.qml" >/dev/null
[[ -s "$APOLLO_SNAPSHOT_PATH" ]]
printf '%s\n' "$APOLLO_SNAPSHOT_PATH"
