...
export APOLLO_CONFIG_HOME="$XDG_CONFIG_HOME/apollo"
export APOLLO_RUNTIME_HOME="$test_root/runtime"
export APOLLO_GENERATED_HOME="$test_root/generated"
...
assert test ! -e "$APOLLO_CONFIG_HOME/matugen"
generate > "$test_root/generated.jsonl"
assert test -s "$APOLLO_GENERATED_HOME/apollo/colors.json"
...
assert cmp "$test_root/ghostty.conf" "$APOLLO_CONFIG_HOME/matugen/templates/ghostty.conf"
...
reject add injection "$test_root/ghostty.conf" '$(touch /tmp/apollo-should-not-exist)' ''
...
cat >> "$APOLLO_CONFIG_HOME/matugen/config.toml" <<'TOML'
[templates.missing]
...
cp "$APOLLO_CONFIG_HOME/matugen/config.toml" "$test_root/before"
cat >> "$APOLLO_CONFIG_HOME/matugen/config.toml" <<'TOML'
[templates.inline]
...
cp "$APOLLO_CONFIG_HOME/matugen/config.toml" "$test_root/invalid"
reject add cannot-add "$test_root/ghostty.conf" '~/another' ''
assert cmp "$APOLLO_CONFIG_HOME/matugen/config.toml" "$test_root/invalid"
cp "$test_root/before" "$APOLLO_CONFIG_HOME/matugen/config.toml"

manage remove manual > /dev/null
manage remove ghostty > /dev/null
assert test ! -e "$APOLLO_CONFIG_HOME/matugen/templates/ghostty.conf"
assert cmp "$HOME/ghostty/Matugen" "$test_root/retained"
...
installed="$test_root/package/etc/xdg/quickshell/apollo"
...
for number in $(seq 1 100); do
    printf '\n[templates.application-%s]\ninput_path = "%s"\noutput_path = "~/application-%s/theme"\n' "$number" "$test_root/ghostty.conf" "$number" >> "$APOLLO_CONFIG_HOME/matugen/config.toml"
done
